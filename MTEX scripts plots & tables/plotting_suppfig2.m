...
%% Plot Supplementary Figure 2 (IQ maps)
...
% exportgraphics(f,[ datastruct(mapn,2).mapname(2:end-9) '_sf2_IQ_undistort.png'],'Resolution',300)
...
%% Plot Supplementary Figure 2 (IQ maps with overlain IPF color)
setMTEXpref('showCoordinates','on')
setMTEXpref('showMicronBar','off')
...
ipfKeyD=ipfHSVKey(ebsd('Diopside'));
ipfKeyD.inversePoleFigureDirection = zvector;
colorCpx=ipfKeyD.orientation2color(ebsd('Diopside').orientations);

ipfKeyM=ipfHSVKey(ebsd('Magnetite'));
ipfKeyM.inversePoleFigureDirection = zvector;
colorTmt=ipfKeyM.orientation2color(ebsd('Magnetite').orientations);

figure,plot(ebsd,ebsd.iq)
mtexColorMap black2white
hold on
plot(ebsd('Magnetite'),colorTmt,'FaceAlpha',0.88)
hold on
plot(ebsd('Diopside'),colorCpx,'faceAlpha',0.45)
if ~isempty(ebsd('Eskolaite'))
hold on
plot(ebsd('Eskolaite'),'FaceAlpha',0.5,'FaceColor','LightBlue')
end
ax=gca;
legend(ax,'off')
...
% f = gcf;
% f.Position = [leftfrac*w botfrac*h horzfrac*w horzfrac*w*aratio];
% exportgraphics(f,[ datastruct(mapn,2).mapname(2:end-9) '_sf2_IQ_undistort_IPF.png'],'Resolution',300)
